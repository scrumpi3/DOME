 ...
 *
 * $Id: UnitTab.java,v 1.2 2004/04/26 11:29:31 elaine Exp $
 */
 ...
 * Prefix, UnitAtom, etc.
 *
 * Currently there can only be one system of units active at the
 * same time, because the system definition is scattered throughout
 * the package in static global variables.
 *
 * @author Gunther Schadow
 * @version $Id: UnitTab.java,v 1.2 2004/04/26 11:29:31 elaine Exp $
 * */
public class UnitTab {
	        ...
	        throws IOException
	{
		try {
			...
			throw new IllegalArgumentException("unit file:" + x.line + ": " + x.getMessage());
		}
	}
		...
		while (stok.ttype != StreamTokenizer.TT_EOF) {
			...
			if (stok.sval.equalsIgnoreCase("case")) {
				...
				if (stok.ttype == StreamTokenizer.TT_WORD) {
					...
					if (stok.sval.equalsIgnoreCase("nonmetric")) {
						// System.out.println("... turns off the metric flag");

						metric = false;
					} else if (stok.sval.equalsIgnoreCase("metric")) {
