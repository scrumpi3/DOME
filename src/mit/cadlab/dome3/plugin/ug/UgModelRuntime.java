 ...
 */
public class UgModelRuntime extends PluginModelRuntime
{
	...
	protected UgPlugin plg;

	public UgModelRuntime(CompoundId id, Element xml, boolean isProjectResource)
	{
	...
	protected void loadNativeModel()
	{
		...
		plg = new UgPlugin(UgConfiguration.UGDLL, directoryName, modelName);
		plg.createModel();
			...
			if (tempObject instanceof Parameter)
			{
				...
				else
				{
					if (input)
					{
						...
						((UgDimensionIn) ugPartDataObject).setIsResult(false);
					}
					else
