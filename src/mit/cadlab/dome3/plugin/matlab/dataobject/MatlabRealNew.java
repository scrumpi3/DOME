...
public class MatlabRealNew extends AbstractPluginData
{
	protected MatlabPluginCaller caller;
	...
	// object
	public MatlabRealNew(MatlabPluginCaller caller, long modelPtr, String name)
	{
		this(caller, modelPtr, name, null);
	}

	public MatlabRealNew(MatlabPluginCaller caller, long modelPtr, String name, Parameter realParam)
	{
		this.caller = caller;
		this.parameter = realParam;
		String[] arr = new String[1];
		arr[0] = name;
		exRealObj = caller.callObjectFunc (modelPtr, MatlabPluginCaller.MODEL_CREATE_REAL, arr);
		if (exRealObj == -1) {
	...
	public void destroy()
	{
		if (exRealObj != 0) {
			...
			//caller.callVoidFunc (exRealObj, MatlabPluginCaller.REAL_DESTROY, null);
			exRealObj = 0;
		}
	}
	...
	public double getValue(boolean isNativeCall)
	{
		...
		return caller.callDoubleFunc(exRealObj, MatlabPluginCaller.REAL_GET_VALUE, null);
	}
	...
	public void setValue(double value, boolean isNativeCall)
	{
		...
		caller.callVoidFunc(exRealObj, MatlabPluginCaller.REAL_SET_VALUE, arr);
	}
