 ...
 */
public class ExcelModelRuntimeNew extends PluginModelRuntime {
    protected ExcelPluginNew plg;

    public ExcelModelRuntimeNew(CompoundId parentId, Element xml, boolean isProjectResource) {
        super(parentId, xml, isProjectResource);
        loadNativeModel();
	    System.out.println("--------------------Excel plugin of new architecture!------------------------------ ");
    }
    ...
    protected void loadNativeModel() {
        ...
        Debug.trace(Debug.ALL,"ExcelModelRuntimeNew:loadNativeModel");
        Debug.trace(Debug.ALL,"ExcelModelRuntimeNew:loadNativeModel excelVersion = " + excelVersion);
        Debug.trace(Debug.ALL,"ExcelModelRuntimeNew:loadNativeModel excelDllName = " + excelDllName);
        ...
        if (fileName == null)
            throw new UnsupportedOperationException("cannot start Excel model - no filename");
        
        plg = new ExcelPluginNew(excelDllName, fileName, runInForeground);
        
     	System.out.println("ExcelModelRuntimeNew:loadNativeModel after ExcelPluginNew " );
       
        plg.createModel();
    	System.out.println("ExcelModelRuntimeNew:loadNativeModel after createModel() " );
    ...
    protected void createExcelLink (Parameter p, String refString, boolean isInput)
    {
	    ...
	    if (p.getCurrentType().equals(DomeReal.TYPE_INFO.getTypeName())) {
		    ...
		    if (!isInput)
			    ((ExcelRealNew) xlObj).setIsResult(true);
	    }
	    
	    else if (p.getCurrentType().equals(DomeString.TYPE_INFO.getTypeName())) {
		    xlObj = (plg).createString(refData[0], refData[1], p);
		    if (!isInput)
			    ((ExcelStringNew) xlObj).setIsResult(true);
	    }
