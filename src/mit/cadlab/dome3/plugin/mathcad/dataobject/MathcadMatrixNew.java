...
public class MathcadMatrixNew extends AbstractPluginData
{
	...
	public MathcadMatrixNew(MathcadPluginCaller caller, long modelPtr, String name, int rows, int cols)
	{
		this(caller, modelPtr, name, rows, cols, null);
	}

	public MathcadMatrixNew(MathcadPluginCaller caller, long modelPtr, String name, int rows, int cols, Parameter matrixParam)
	{
		this.caller = caller;
		...
		exMatrixObj = caller.callObjectFunc(modelPtr, MathcadPluginCaller.MODEL_CREATE_MATRIX, arr);
		if (exMatrixObj == -1) {
	...
	{
		if (exMatrixObj != 0) {
			//caller.callVoidFunc (exRealObj, MathcadPluginCaller.MATRIX_DESTROY, null);
			exMatrixObj = 0;
		}
	}
	...
	public void setDimension(int rows, int cols, boolean isNativeCall)
	{
		...
		caller.callVoidFunc(exMatrixObj, MathcadPluginCaller.MATRIX_SET_DIM, arr);
	}
	...
	public int getRows(boolean isNativeCall)
	{
		...
		return caller.callIntFunc(exMatrixObj, MathcadPluginCaller.MATRIX_GET_ROWS, null);
	}
	...
	public void setRows(int rows, boolean isNativeCall)
	{
		...
		arr[0] = new Integer(rows);
		caller.callVoidFunc(exMatrixObj, MathcadPluginCaller.MATRIX_SET_ROWS, arr);
	}
	...
	public int getColumns(boolean isNativeCall)
	{
		...
		return caller.callIntFunc(exMatrixObj, MathcadPluginCaller.MATRIX_GET_COLS, null);
	}
	...
	public void setColumns(int cols, boolean isNativeCall)
	{
		...
		caller.callVoidFunc(exMatrixObj, MathcadPluginCaller.MATRIX_SET_COLS, arr);
	}
	...
	public double[][] getValues(boolean isNativeCall)
	{
		...
		return caller.call2DimDoubleArrayFunc(exMatrixObj, MathcadPluginCaller.MATRIX_GET_VALUES,
		                                      null);
	}
	...
	public void setValues(double[][] values, boolean isNativeCall)
	{
		...
		caller.callVoidFunc(exMatrixObj, MathcadPluginCaller.MATRIX_SET_VALUES, arr);
	}
	...
	public double getElement(int row, int col, boolean isNativeCall)
	{
		...
		double result =  caller.callDoubleFunc(exMatrixObj, MathcadPluginCaller.MATRIX_GET_ELEMENT,
		                                       arr);
		return result;
	}
	...
	public void setElement(int row, int col, double value, boolean isNativeCall)
	{
		...
		caller.callVoidFunc(exMatrixObj, MathcadPluginCaller.MATRIX_SET_ELEMENT, arr);
	}
	...
	{
		return ("MathcadMatrix: " + exMatrixObj);
	}
}