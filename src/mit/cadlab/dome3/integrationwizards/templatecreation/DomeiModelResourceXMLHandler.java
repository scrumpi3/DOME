 ...
 */
public class DomeiModelResourceXMLHandler {
    public IModelData getiModelData(String xmlString) throws FileHandlerException {
        IModelData data = null;
        try {
            data = loadiModel(xmlString);
        } catch (IOException e) {
            e.printStackTrace();
        }
        return data;
    ...
    public IModelData loadiModel(String xmlString) throws IOException {
        ...
        for (Iterator iterator = nodesXML.iterator(); iterator.hasNext();) {
            ...
            if (modelobjectsMap.get(NodeId) != null) {//sometimes the interface file xml is screwed up may has bad node idref,
                ...
                if (outputs.contains(p)) {
                    AttributedNode n = graph.addNode(p.getName(), p.getDataType(), p.getUnit(), p.getDimension(), "output",1);
                    param_node_map.put(p, n);
                } else if (inputs.contains(p)) {
                    AttributedNode n = graph.addNode(p.getName(), p.getDataType(), p.getUnit(), p.getDimension(),"input",1);
                    param_node_map.put(p, n);}
                 else {
                    AttributedNode n = graph.addNode(p.getName(), p.getDataType(), p.getUnit(), p.getDimension(),null,1);
                    param_node_map.put(p, n);
                }
            }
        }
