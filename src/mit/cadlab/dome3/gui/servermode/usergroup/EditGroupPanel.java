...
public class EditGroupPanel
{
	...
	private static final Dimension DEFAULT_SIZE = new Dimension(300, 210);

	private JPanel gui;
	private JTextField namefield;
	private JTextField descriptionfield;
	private JButton members = Templates.makeButton("view/edit members");
	...
	private boolean editedGroup = false;

	public static boolean showPanel(Component parent, ServerConnection svrConn, Vector data)
	{
		EditGroupPanel np = new EditGroupPanel(svrConn, data);
		JDialog d = DialogFactory.createDialog(parent, "Edit Group", np.gui, true, false);
		d.setDefaultCloseOperation(WindowConstants.DISPOSE_ON_CLOSE);
		d.show();
		return np.editedGroup;
	}

	public EditGroupPanel(ServerConnection svrConn, Vector data)
	{
	...
	public EditGroupPanel(ServerConnection svrConn, int id, String name, String desc,
	...
	private JPanel makeDataPanel()
	{
		...
		JComponent[] comps = {Templates.makeLabel("name:"), namefield,
		                      ...
		                      members, saveModelCheckBox, savePlayspaceCheckBox,
		...
		GridBagConstraints[] gbcs = {
			...
			new GridBagConstraints(1, 3, 1, 1, 1.0, 0.0, gbc.WEST, gbc.NONE, new Insets(5, 5, 0, 5), 0, 0),
			new GridBagConstraints(1, 4, 1, 1, 1.0, 0.0, gbc.WEST, gbc.NONE, new Insets(2, 5, 0, 5), 0, 0),
			new GridBagConstraints(0, 5, 1, 1, 0.0, 0.0, gbc.WEST, gbc.NONE, new Insets(5, 5, 0, 0), 0, 0),
			new GridBagConstraints(1, 5, 1, 1, 1.0, 0.0, gbc.WEST, gbc.NONE, new Insets(5, 5, 0, 5), 0, 0),
			new GridBagConstraints(1, 6, 1, 1, 1.0, 1.0, gbc.SOUTHEAST, gbc.NONE, new Insets(5, 5, 5, 5), 0, 0),
		};
		...
		members.addActionListener(new ActionListener()
		{
			...
			{
				ViewEditMembersForGroupPanel.showPanel(EditGroupPanel.this.gui, svrConn, id, name);
			}
		});
	...
	private void editGroup()
	{
		try {
			...
			OneButton1Msg.showError(gui, "Edit group error", ex.getMessage(), "ok",
