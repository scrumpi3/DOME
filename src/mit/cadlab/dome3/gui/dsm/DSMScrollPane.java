...
import mit.cadlab.dome3.swing.CompositeIcon;
...
{
	protected final static Dimension DEFAULT_SIZE = new Dimension(300, 300);

	int n;                    //mit.cadlab.dome3.gui.dsm Size
	int[][] originalMatrix;  //Original Matrix
	Object[][] sortedMatrix;  //Matrix that mit.cadlab.dome3.gui.dsm currently show--change to object for show
	...
	{
		super();
		setData(nMatrix, columns, activities);
	}

	public void setData(int[][] nMatrix, String[] columns, String[] activities)
	{
		//initialize
		n = nMatrix.length;
		...
		//initial draw
		drawDSM(originalMatrix, originalColumnNumbers, _originalActivities);
	...
	public void setData(int[][] nMatrix, String[] activities)
	{
		...
		// check possibly input error here:
		if (nMatrix.length != activities.length)
			System.out.println("incompatiable!!!" + nMatrix.length + "," + activities.length);
		String[] ns = new String[n];
		for (int k = 0; k < n; k++) {
			ns[k] = new Integer(k + 1).toString();
		}
		setData(nMatrix, ns, activities);
	...
	public void drawDSM(int[][] matrixData, String[] columnNumbers, String[] activityNames)
	{
		...
		setPreferredSize(tableView.getPreferredSize());
	...
	protected void createRowHeaderTable()
	{
		...
		try {
			JTable rowTable = new JTable(new RowHeaderModel());
			...
			rowviewport.setView(rowTable);
			rowviewport.setPreferredSize(rowTable.getPreferredSize());
			rowviewport.setBackground(this.getBackground());
			this.setRowHeaderView(rowviewport);
		} catch (Exception ex) {
			ex.printStackTrace();
		}
	}
	...
	protected void createColumnHeaderTable()
	{
		try {
			...
			JTable columnTable = new ObjectTable(new DsmDataObjectTableModel(testData, this._sortedActivities.length, this.getBackground()));
			for (int i = 0; i < this.sortedColumnNumbers.length; i++) {
			...
			columnTable.setRowHeight(0, maxHeight + 20);
			columnTable.setRowHeight(1, 20);
			columnTable.setPreferredSize(new Dimension(this._sortedActivities.length * 20, maxHeight + 40));
			columnTable.setBackground(this.getBackground());
			columnTable.setOpaque(true);

			JViewport columnviewport = new JViewport();
			columnviewport.setView(columnTable);
			columnviewport.setPreferredSize(columnTable.getPreferredSize());
			columnviewport.setBackground(this.getBackground());
			this.setColumnHeaderView(columnviewport);
