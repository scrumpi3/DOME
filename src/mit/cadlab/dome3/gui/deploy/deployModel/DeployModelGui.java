...
/**
 * The main class for the model deploy wizard
 */
public class DeployModelGui extends JPanel implements ActionListener
{
	...
	private JRadioButton selModel;
	private JRadioButton selLocation;
	private JRadioButton setModelPriv;
	private JRadioButton selInterfaces;
	private JRadioButton setInterfacePriv;
	...
	{
		(data.getSelectModelCard()).setToNewDeploy();
	}
	...
	/**
	 * GUI class to run the deploy wizard
	 * @param deployModel the DeployModel data
	 */
	public DeployModelGui(DeployModel deployModel)
	{
		data = deployModel;
	...
	private JPanel makeRadioPanel()
	{
		...
		selModel = Templates.makeRadioButton("Select model", false);
		selModel.setBackground(Templates.DARKER_BACKGROUND_COLOR);
		//selModel.setEnabled(false);

		selLocation = Templates.makeRadioButton("Select location on server", false);
		selLocation.setBackground(Templates.DARKER_BACKGROUND_COLOR);
		//selLocation.setEnabled(false);

		setModelPriv = Templates.makeRadioButton("Set model editing privileges", false);
		setModelPriv.setBackground(Templates.DARKER_BACKGROUND_COLOR);
		//setModelPriv.setEnabled(false);

		selInterfaces = Templates.makeRadioButton("Select interfaces to deploy", false);
		selInterfaces.setBackground(Templates.DARKER_BACKGROUND_COLOR);
		//selInterfaces.setEnabled(false);

		setInterfacePriv = Templates.makeRadioButton("Set interface use privileges", false);
		setInterfacePriv.setBackground(Templates.DARKER_BACKGROUND_COLOR);
		//setInterfacePriv.setEnabled(false);
		...
		JComponent[] comps = {login, selModel, selLocation, setModelPriv, selInterfaces,
		                      setInterfacePriv, confirm, fill};
		// gridx, gridy, gridwidth, gridheight, weightx, weighty, anchor, fill, insets(t,l,b,r), ipadx, ipady
		GridBagConstraints[] gbcs = {
		...
		JRadioButton[] buttonsTemp = new JRadioButton[]{login, selModel, selLocation, setModelPriv,
		                                                selInterfaces, setInterfacePriv, confirm};
		buttons = buttonsTemp;
		return p;
	}
	...
	private JPanel makeCardPanel()
	{
		...
		cardPanel.add("selModel", new SelectModelCard(data, this));
		cardPanel.add("selLocation", new SelectLocationCard(data, this));
		cardPanel.add("setModelPriv", new SetModelPrivCard(data, this));
		cardPanel.add("selInterfaces", new SelectInterfaceCard(data, this));
		cardPanel.add("setInterfacePriv", new SetInterfacePrivCard(data, this));
	...
	private void initializeDeploySequence()
	{
		...
		//reset gui fields in the differentPanels
		//data.getLoginCard().initGui(); do not want to initialize so that second time through don't have to login again
		//but need the hack below to get new serverPanel. Look at playspace implementation. Only needed here because of the
		//poor structure. Need to rewrite deploy model folliwng pattern of deploy playspace so code is more maintainable
		if (data.getServerConnection() != null)
		...
		data.getSelectLocationCard().initGui();
		data.getSelectInterfaceCard().initGui();
		data.getSetInterfacePrivCard().initGui();
	...
	private void setNextCard()
	{
		if (sel != buttons.length - 1) {
			...
			sel++;
			// this is needed because the if redeploying location is automatically selected
			if ((sel == SEL_LOCATION) && (!data.getNewDeployment()))
				successfulCompletion();
			// this is needed because the model priv card does not require input before going further
			else if (sel == MODEL_PRIV)
				successfulCompletion();
			else if (sel == INTERFACE_SEL) {
				if (data.getModelData().getNumAvailable() > 0) successfulCompletion();
			} else if (sel == INTERFACE_PRIV)
				data.getSetInterfacePrivCard().setInterfacePermPanel();
			else if (sel == CONFIRM)
				data.getConfirmCard().setConfirmChoicesData();
			buttons[sel].setEnabled(true);
			buttons[sel].setSelected(true);
	...
	public void dispose()
	{
		SwingUtilities.windowForComponent(DeployModelGui.this).dispose();
	}
	...
	{
		JFrame f = new JFrame("Deploy model");
		f.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
		f.getContentPane().add(new DeployModelGui(null));
