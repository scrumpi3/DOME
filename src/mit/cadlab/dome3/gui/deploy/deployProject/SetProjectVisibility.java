...
public class SetProjectVisibility extends JPanel
{
	...
	public void setProjectVisCard()
	{
		...
		if (data.isNewDeployment()) {
			panel = new PermissionsPanel(data.getServerConnection(), permissionCategory, null, data.getProjectData().getName(), false, false);
		...
		} else if (PermissionFunctions.sessionUserHasPermission(data.getServerConnection(), data.getRedeployProjectId(), PermissionUtils.PERMISSION_TO_SET_PROJECT_VISIBILITY_PRIVS)) {
			ServerConnection svr = data.getServerConnection();
			panel = new PermissionsPanel(svr, permissionCategory, data.getRedeployProjectId(), data.getProjectData().getName(), true, false);
			setVisibilityCard.add("visibility permissions", panel.getGui());
			((CardLayout2) setVisibilityCard.getLayout()).last(setVisibilityCard);
			data.setVisibilityPermissionsPanel(panel);
		} else {
	...
	public SetProjectVisibility(DeployProject deployData, DeployProjectGui gui)
	{
		data = deployData;
		deployGui = gui;
