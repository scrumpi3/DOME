...
public class LocateCard extends JPanel
{
	...
	public void setServerPanelCard()
	{
		...
		if (!data.isNewDeployment()) { //redeploy case so need to set selection to correct playspace
			String root = DeployFilesFunctions.getRootForPlayspace(data.getServerConnection(), data.getRedeployPlayspaceId());
			...
			serverPanel.setObjectSelection(data.getRedeployPlayspaceId(), LocateCard.PLAYSPACE); //this also locks the table selection to this item
	...
	{
		JPanel p = new JPanel();
		JLabel msg1 = Templates.makeLabel("Where will the playspace be located the server?", Templates.FONT12B);
