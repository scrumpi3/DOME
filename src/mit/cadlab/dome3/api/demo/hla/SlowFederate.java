...
public class SlowFederate extends FederateAmbassadorImpl {
    public static final String ID = "SLOW_FED";
    ...
    private LogicalTimeIntervalDouble interval = new LogicalTimeIntervalDouble(0.5);
    private LogicalTimeIntervalDouble halfInterval = new LogicalTimeIntervalDouble(0.25);
    private static final int CRC_PORT = 8989;

    /** (ex) new SlowFederate("000", "C:/dome3/out/WarField.xml"); */
    public SlowFederate(String sessionId, String filePath) {
    ...
    public void setup(String filePath) {
        ...
        try {
            absd.modifyLookahead(halfInterval);
...
//                    absd.timeAdvanceRequest(theNextTime);
                    ...
                    System.out.println("[SLOW] waiting... after requesting time advance: " + theNextTime);
                    reservationSemaphore.wait();
                    theTime = theNextTime;
                }

                double theTimeValue = theTime.getValue();
                if (theTimeValue == 6) { sendInteractionMessageTimeStamped("[SLOW] send result #1"); }
                if (theTimeValue == 7) { sendInteractionMessageTimeStamped("[SLOW] send result #2"); }
                if (theTimeValue == 12) { sendInteractionMessageTimeStamped("[SLOW] send result #3"); }
                if (theTimeValue == 13) { sendInteractionMessageTimeStamped("[SLOW] send result #4"); }
    ...
    public void timeConstrainedEnabled(LogicalTime theFederateTime) {
        System.out.println("[SLOW] time constrained enabled: " + theFederateTime);
        synchronized (reservationSemaphore) {
    ...
    public void timeRegulationEnabled(LogicalTime theFederateTime) {
        System.out.println("[SLOW] time regulation enabled: " + theFederateTime);
        synchronized (reservationSemaphore) {
            reservationSemaphore.notify();
        }
    }
    ...
    public void timeAdvanceGrant(LogicalTime theFederateTime) {
        System.out.println("[SLOW] time advance granted: " + theFederateTime);
        synchronized (reservationSemaphore) {
            reservationSemaphore.notify();
        }
    }
    ...
    public final void receiveInteraction(InteractionClassHandle interaction, ParameterHandleValueMap valueMap, byte[] bytes, OrderType orderType, TransportationType transportationType, LogicalTime logicalTime, OrderType orderType1) {
        synchronized (reservationSemaphore) {
            if (interaction.equals(CID_COMMUNICATION)) {
                ...
                if (FastFederate.ID.equals(sender) && SlowFederate.ID.equals(receiver)) {
                    try {
                        System.out.println("[SLOW] received " + message + " time-stamped " + logicalTime + " at " + theTime);
                    } catch (Exception e) { e.printStackTrace(); }
                }
            }
        }
    ...
    public void sendInteractionMessage(String msg, LogicalTime time) {
        try {
            ...
            parameters.put(PID_SENDER, SlowFederate.ID.getBytes());
            parameters.put(PID_RECEIVER, FastFederate.ID.getBytes());
