...
import mit.cadlab.dome3.plugin.mathematica.MathematicaModelRuntime;
 ...
 */
public class RunMathematicaModel
{
	public static DomeFileChooser fileChooser = new DomeFileChooser();

	public static MathematicaModelRuntime loadMathematicaModel() {
		DomeFileChooser fc = new DomeFileChooser();
		String localFileName = fc.showOpenDialog(null, MathematicaConfiguration.TYPE_INFO.getTypeName());
		if (localFileName == null)
			return null; // cancelled
		// construct the model and a frame for it
		Element modelElement = XMLUtils.fileToXmlElement(localFileName);
		MathematicaModelRuntime model = new MathematicaModelRuntime(new CompoundId(), modelElement, false);
		return model;
	}

	public static ModelInterfaceRuntimeServer loadInterfaceForModel(MathematicaModelRuntime model) {
	...
	public static void testMathematicaModel() {
		MathematicaModelRuntime model = loadMathematicaModel();
		if (model==null)
			return;
