...
public class ClosingGui extends JPanel implements ActionListener
{
    ...
    public ClosingGui(ModelInterfaceBase iface) {
        this();
        setInterface(iface);
    }
	...
	private JPanel makeRadioPanel()
	{
		...
		JComponent[] comps = {inputLabel, closingSpring, resultLabel,
        ...
        buttons[CLOSING_INDEX] = closingSpring;
	...
	{   //cards must be added in the same order as the button group indices
		...
		cardPanel.add(CLOSING_CARD, new ClosingCard());
	...
	public void actionPerformed(ActionEvent event)
	{
		...
		if (object == backButton) {
            ...
            cardIndex = CLOSING_INDEX;
            ((CardLayout2) cardPanel.getLayout()).show(cardPanel, CLOSING_CARD);
        }
	...
	public void dispose()
	{
		SwingUtilities.windowForComponent(ClosingGui.this).dispose();
	}
    ...
    private void setInterface(ModelInterfaceBase iface) {
        ((ClosingCard) ((CardLayout2) (cardPanel.getLayout())).getComponent(CLOSING_CARD)).setInterface(iface);
	...
	public static void main(String[] args)
	{
		...
		f.getContentPane().add(new ClosingGui());
