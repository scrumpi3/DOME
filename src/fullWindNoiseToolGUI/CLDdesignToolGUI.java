 ...
 */
public class CLDdesignToolGUI extends JPanel implements ActionListener
{
	public static final GridBagConstraints gbc = null;

    private static final String SEAL_CARD = "sealGeometry";
    private static final String DOOR_CARD = "doorGeometry";
    private static final String CLD_CARD = "doorSealCLD";
    private static final String INTERFERENCE_SETUP_CARD = "interfernceSetup";
    ...
    private static final int CLD_INDEX = 2;
    private static final int INTERFERENCE_SETUP_INDEX = 3;

    private static final int INTERFERENCE_RESULT_INDEX = 4;
    ...
    private JRadioButton doorCldSetup;
    ...
    public CLDdesignToolGUI(ModelInterfaceBase iface){
        this();
        setInterface(iface);
    }
	...
	private JPanel makeRadioPanel()
	{
        ...
        doorCldSetup = Templates.makeRadioButton("Door seal CLD", false);
        doorCldSetup.setBackground(Templates.DARKER_BACKGROUND_COLOR);
        doorCldSetup.addActionListener(this);
		...
		JComponent[] comps = {inputLabel, sealGeometry, doorGeometry,  doorCldSetup,
                              //latch, air, closingSpring,
                              interferenceSetup,
                              resultLabel,
                              //feaResults,
                              interferenceResults, standardResults, teardownResults, fill, imageLabel};
		// gridx, gridy, gridwidth, gridheight, weightx, weighty, anchor, fill, insets(t,l,b,r), ipadx, ipady
		GridBagConstraints[] gbcs = {
        ...
        buttons[CLD_INDEX] = doorCldSetup;
	...
	{   //cards must be added in the same order as the button group indices
		...
		cardPanel.add(SEAL_CARD, new SealGeometryForCLDCard());
        cardPanel.add(DOOR_CARD, new DoorGeometryCard());
		cardPanel.add(CLD_CARD, new DoorSealCLDCard());
	...
	public void actionPerformed(ActionEvent event)
	{
        ...
        else if (object == doorCldSetup) {
            cardIndex = CLD_INDEX;
            ((CardLayout2) cardPanel.getLayout()).show(cardPanel, CLD_CARD);
        }
	...
	public void dispose()
	{
		SwingUtilities.windowForComponent(CLDdesignToolGUI.this).dispose();
	}
    ...
    private void setInterface(ModelInterfaceBase iface){
       ((SealGeometryForCLDCard)((CardLayout2)(cardPanel.getLayout())).getComponent(SEAL_CARD)).setInterface(iface);
       ((DoorGeometryCard) ((CardLayout2) (cardPanel.getLayout())).getComponent(DOOR_CARD)).setInterface(iface);
       ((DoorSealCLDCard) ((CardLayout2) (cardPanel.getLayout())).getComponent(CLD_CARD)).setInterface(iface);
	...
	{
		JFrame f = new JFrame("Integrated tool custom GUI for manual cld definition");
		f.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
		f.getContentPane().add(new CLDdesignToolGUI());
