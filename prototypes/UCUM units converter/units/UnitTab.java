...
public static void readMore(String src) throws IOException {
  ...
  { 
	...
	if(stok.sval.equalsIgnoreCase("case")) {
	  ...
	  Unit base = new UnitAtom(stok.sval, 1, new Dimension(dimension++));
	  
	  if(verbose)
	    { System.out.print("base\t"); base.dump(); }
	  ...
	  try {
	    ...
	    Unit lunit = new UnitAtom(symbol, runit, cnv, 1, metric);
	    ...
	    if(verbose)
	      {System.out.print("derived\t"); lunit.dump(); }
	  }
	  catch(Exception x) {
...
class Conflict {
  ...
  static void init() { list = new Vector(); }
  ...
  Conflict(String _p1, String _u1, String _p2, String _u2) {
    ...
    if(_u1.length() >= _u2.length())
      { p1 = _p1; p2 = _p2; u1 = _u1; u2 = _u2; }
    else
      { p1 = _p2; p2 = _p1; u1 = _u2; u2 = _u1; }
