...
public class InterferenceGui extends JPanel implements ActionListener
{
	...
	private JPanel makeRadioPanel()
	{
        ...
        geometry = Templates.makeRadioButton("geometric configuration", false);
        geometry.setBackground(Templates.DARKER_BACKGROUND_COLOR);
        ...
        ImageIcon image = Templates.makeImageIcon("interferenceGUIwithGeometry/images/interferenceLogo.gif");
		...
		GridBagConstraints[] gbcs = {
            ...
            new GridBagConstraints(0, 3, 1, 1, 0.0, 0.0, gbc.WEST, gbc.NONE, new Insets(10, 5, 0, 5), 0, 0),

            new GridBagConstraints(0, 4, 1, 1, 0.0, 0.0, gbc.WEST, gbc.HORIZONTAL, new Insets(5, 5, 0, 5), 0, 0),

            new GridBagConstraints(0, 5, 1, 1, 0.0, 1.0, gbc.WEST, gbc.VERTICAL, new Insets(0, 0, 0, 0), 0, 0),
			new GridBagConstraints(0, 6, 1, 1, 0.0, 0.0, gbc.SOUTHWEST, gbc.NONE, new Insets(0, 0, 0, 0), 0, 0)
		};
		Templates.layoutGridBag(p, comps, gbcs);
		p.setBackground(Templates.DARKER_BACKGROUND_COLOR);

        buttonGroup = new ButtonGroup();
        buttonGroup.add(geometry);
        buttonGroup.add(config);
        buttonGroup.add(interferenceResults);

		buttons = new JRadioButton[NUM_CARD];
        buttons[GEOM_INDEX] = geometry;
	...
	{   //cards must be added in the same order as the button group indices
        ...
        cardPanel.add(GEOM_CARD, new GeometryCard());
	...
	public void actionPerformed(ActionEvent event)
	{
		...
		if (object == backButton) {
            ...
            cardIndex = GEOM_INDEX;
            ((CardLayout2) cardPanel.getLayout()).show(cardPanel, GEOM_CARD);
        }
    ...
    private void setInterface(ModelInterfaceBase iface) {
        ((GeometryCard) ((CardLayout2) (cardPanel.getLayout())).getComponent(GEOM_CARD)).setInterface(iface);
	...
	public static void main(String[] args)
	{
		...
		f.setSize(InterferenceGui.DEFAULT_SIZE);
		f.show();
	}
}
