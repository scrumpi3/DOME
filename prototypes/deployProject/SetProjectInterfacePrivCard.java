...
import mit.cadlab.dome.gui.serverPanel.ServerPanelComboBoxModel;
...
public class SetProjectInterfacePrivCard extends JPanel
{
	...
	{
		if (data.getProjectData().getNumAvailable()>0) {
			...
			DeployInterfaceData[] intData =
			        (DeployInterfaceData[]) (data.getProjectData().getInterfaces().toArray(new DeployInterfaceData[]{}));

			((CardLayout2) interfaceUsePrivPanel.getLayout()).show(interfaceUsePrivPanel, INTERFACES_AVAILABLE);
			if (!((data.getProjectInterfacePermissions() == null) || (data.getProjectInterfacePermissions().isEmpty()))) resetInterfacePriv();
			...
			else if (PermissionFunctions.sessionUserHasPermission(svr, data.getRedeployProjectId(), PermissionUtils.PERMISSION_TO_SET_INTERFACE_USE_PRIV)) {
				for (int i = 0; i < intData.length; i++) {
	...
	public SetProjectInterfacePrivCard(DeployProject deployData, DeployProjectGui gui)
	{
		data = deployData;
		deployGui = gui;
		data.setInterfaceUsePrivCard(this);
