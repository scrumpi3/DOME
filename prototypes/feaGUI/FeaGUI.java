 ...
 */
public class FeaGUI extends JPanel implements ActionListener
{
    ...
    public FeaGUI(ModelInterfaceBase iface) {
        this();
        setInterface(iface);
    }
	...
	private JPanel makeRadioPanel()
	{
        ...
        config = Templates.makeRadioButton("geometry and material", false);
        config.setBackground(Templates.DARKER_BACKGROUND_COLOR);
        config.addActionListener(this);

        results = Templates.makeRadioButton("seal analysis", false);
        results.setBackground(Templates.DARKER_BACKGROUND_COLOR);
        ...
        ImageIcon image = Templates.makeImageIcon("feaGUI/logo.gif");
	...
	{   //cards must be added in the same order as the button group indices
        ...
        cardPanel.add(RESULT_CARD, new ResultCard());
	...
	public void dispose()
	{
		SwingUtilities.windowForComponent(FeaGUI.this).dispose();
	}
    ...
    private void setInterface(ModelInterfaceBase iface) {
        ...
        ((ResultCard) ((CardLayout2) (cardPanel.getLayout())).getComponent(RESULT_CARD)).setInterface(iface);
    }

	public static void main(String[] args)
	{
		JFrame f = new JFrame("2D seal non-linear FEA custom GUI");
		f.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
		f.getContentPane().add(new FeaGUI());
