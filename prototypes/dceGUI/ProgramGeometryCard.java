 ...
 */
public class ProgramGeometryCard extends JPanel
{
	...
	{
		JPanel p = new JPanel();
		JLabel programLabel = Templates.makeLabel("Program", Templates.FONT12B);
        JTextArea programText = Templates.makeDTextArea("Vehicle program specific information is not used in the analysis");
        programText.setOpaque(false);

        JLabel geometryLabel = Templates.makeLabel("Geometry", Templates.FONT12B);
        JTextArea geometryText = Templates.makeDTextArea("Door type: conventional front door");
        geometryText.setOpaque(false);
        geometryText.setToolTipText("Front-hinged doors with one latch. The front door closes and latches on the rear door.");

        ImageIcon image = Templates.makeImageIcon("dceGUI/images/geometry.gif");
        JLabel imageLabel = new JLabel(image);
        imageLabel.setToolTipText("geometry nomenclature");

		JComponent[] comps = {programLabel, programText, geometryLabel, geometryText, imageLabel};
		// gridx, gridy, gridwidth, gridheight, weightx, weighty, anchor, fill, insets(t,l,b,r), ipadx, ipady
		GridBagConstraints[] gbcs = {
            ...
            new GridBagConstraints(1, 0, 1, 4, 0.0, 0.0, gbc.SOUTHEAST, gbc.NONE, new Insets(5, 5, 0, 5), 0, 0),
		};
    ...
    private JScrollPane makePointPanel()
    {
        ...
        JLabel geometryPointsLabel2 = Templates.makeLabel("Choose geometry points and seal offsets to approximate door seal perimeter.", Templates.FONT12B);
	...
	public static void main(String[] args)
	{
		...
		f.getContentPane().add(new ProgramGeometryCard());
		f.show();
	}
}
