 ...
 */
public class LocateToolCard extends JPanel
{
	...
	private DeployToolGui deployGui; //used later to call the sucessfulCompletion method
	...
	public void setServerPanelCard()
	{
		...
		if (!data.isNewDeployment()) { //redeploy case so need to set selection to correct playspace
			String root = DeployFilesFunctions.getRootForProject(data.getServerConnection(), data.getRedeployToolProjectId());
			...
			serverPanel.setObjectSelection(data.getRedeployToolProjectId(), "project"); //this also locks the table selection to this item
	...
	public LocateToolCard(DeployTool deployData, DeployToolGui gui)
	{
		data = deployData;
		deployGui = gui;
		data.setLocateToolCard(this);
	...
	public static void main(String[] args)
	{
		...
		f.getContentPane().add(new LocateToolCard(null, null));
		f.show();
	}
}
