...
figure; bar([sum(qfentotR+qrwtot+qintotR)' sum(qtrtot+qiftot+qvetotR)']); title('total, actual heat gain and loss');
xlabel('month'); ylabel('(BTU)'); legend('heat gain', 'heat loss');

figure; bar([sum(qfentotR)' sum(qrwtot)' sum(qintotR)']); title('total, actual heat gains');
xlabel('month'); ylabel('(BTU)'); legend('fenestrations', 'roofs and walls', 'internal sources', 0);

figure; bar([sum(qtrtot)' sum(qiftot)' sum(qvetotR)']); title('total, actual heat losses');
xlabel('month'); ylabel('(BTU)'); legend('transmission', 'infiltration', 'ventilation', 0);

figure; bar([sum(qfentot+qrwtot+qintot)' sum(qtrtot+qiftot+qvetot)']); title('heat gain and loss that affect energy consumption');
xlabel('month'); ylabel('(BTU)'); legend('heat gain', 'heat loss');
for mo = 1:12
...
figure; bar([coolE' heatE']); title('net heating and cooling loads');
xlabel('month'); ylabel('(BTU)'); legend('cooling', 'heating');
figure; bar(qwttot'); title('net domestic hot water heating load');
